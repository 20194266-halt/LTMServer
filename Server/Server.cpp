...
void messageHandling(socketInfo &client, socketInfo* sock, DWORD &index, DWORD &nEvents, WSAEVENT* events)
{
	...
	for (int i = 0; i < recvMessage.size(); i++) {
		...
		{
			string listOfRequestReply = getRequestReply(tmpStr, nameAndPass1);
			sendMessage(client.clientSock, listOfRequestReply);
		}
		else if (header == "GETINVITEREPLY")
		{
			string listOfInvitationReply = getInvitationReply(tmpStr, nameAndPass1);
			sendMessage(client.clientSock, listOfInvitationReply);
		}
		else if (header == "USERATTEND")
		{
			string listUser = getUserAttendList(tmpStr, eventNameMap1);
